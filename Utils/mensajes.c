...
void* serializarLocalizedPokemon(t_localized_pokemon* localized_pokemon, uint32_t * bytes) {
	...
	*bytes = pokemon_size + sizeof(uint32_t) + cant_coordenadas * sizeof(t_coords);
	...
	free(serialized_pokemon);
	free_coords_array(coords_array, cant_coordenadas);
...
t_pokemon* deserializarPokemon(t_buffer** buffer) {
	...
	memcpy(pokemon -> name, (*buffer) -> stream, (pokemon -> name_size) - 1);
...
void free_coords_array(t_coords** coords, int cantidad_de_coordenadas) {
	if (coords == NULL)
		return;

	int i = 0;
	while(i < cantidad_de_coordenadas) {
		free(coords[i]);
		i++;
	}
	free(coords);
